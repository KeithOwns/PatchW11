...
$Char_CardIndex   = [char]::ConvertFromUtf32(0x1F5C2)
$Char_Desktop     = [char]::ConvertFromUtf32(0x1F5A5)
$Char_Skip        = [char]::ConvertFromUtf32(0x23ED) # ⏭
...
function Write-TopHeader {
    ...
    Write-Centered $SubTitle -Color "$Bold$FGCyan"
    Write-Host "$FGDarkBlue$([string]$Char_HeavyLine * 60)$Reset"
}

function Write-BodyTitle {
    param([string]$Title)
    Write-LeftAligned "$FGWhite$Char_HeavyMinus $Bold$Title$Reset" -Indent 1
}
...
function Create-RestorePoint {
    ...
    try {
        Checkpoint-Computer -Description "PatchW11 Security Config $(Get-Date -Format 'yyyyMMdd_HHmm')" -RestorePointType "MODIFY_SETTINGS" -ErrorAction Stop
...
function Invoke-ApplySecuritySettings {
    ...
    if ($AutoRun) {
        Write-Host ""
        Write-LeftAligned "$FGYellow AutoRun: Applying security settings...$Reset"
    ...
    Write-Host ""
    $prompt = "${FGWhite}$Char_Keyboard  Press${FGDarkGray} ${FGYellow}$Char_Finger [Enter]${FGDarkGray} ${FGWhite}to${FGDarkGray} ${FGYellow}SET${FGWhite}|${FGDarkGray}any other to ${FGWhite}SKIP$Char_Skip ${Reset}"
...
try {
    ...
    Write-TopHeader "WINDOWS SECURITY CONFIGURATOR"
    
    # NEW: Added Check Sub-header and Boundary per request
    Write-Centered "$Char_EnDash Windows Security CHECK $Char_EnDash" -Color "$Bold$FGDarkCyan"
    Write-Boundary $FGDarkBlue
    ...
    if ($script:ThirdPartyAVActive) {
        $prompt = "${FGGray}$Char_Keyboard  Third-Party AV Active: Scan Skipped$Reset"
        Write-Centered $prompt
    } else {
        ...
        if ($AutoRun) {
            Write-LeftAligned "$FGYellow AutoRun: Initiating $ScanTypeString Scan...$Reset"
            $shouldScan = $true
        } else {
            # EDITED: Updated per request to remove $ScanTypeString from prompt text
            $prompt = "${FGWhite}$Char_Keyboard  Press${FGDarkGray} ${FGYellow}$Char_Finger [Enter]${FGDarkGray} ${FGWhite}to${FGDarkGray} ${FGYellow}RUN Scan${FGWhite}|${FGDarkGray}any other to ${FGWhite}SKIP$Char_Skip ${Reset}"
            Write-Centered $prompt

            $key = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
            if ($key.VirtualKeyCode -eq 13) { $shouldScan = $true }
        }
