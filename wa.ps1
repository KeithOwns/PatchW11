...
- Check Installed State     | Test-WA_AppInstalled
- Install (Winget/MSI/EXE)  | Invoke-WA_InstallApps
________________________________________________________
[C]onfiguration
Security Actions:
- Real-Time Protection      | Invoke-WA_SetRealTimeProt (Set-MpPreference)
- PUA Protection            | Invoke-WA_SetPUABlockApps (Set-MpPreference)
- PUA Protection (Edge)     | Invoke-WA_SetPUABlockDLs (Registry)
- Memory Integrity          | Invoke-WA_SetMemoryInteg (Registry)
...
UI & UX Actions:
- Classic Context Menu      | Invoke-WA_SetClassicMenu (Registry)
- Taskbar Search Box        | Invoke-WA_SetTaskbarSearch (Registry)
- Task View Toggle          | Invoke-WA_SetTaskViewOFF (Registry)
- Microsoft Update Service  | Invoke-WA_SetMicrosoftUpd (Registry)
- Restart Notifications     | Invoke-WA_SetRestartIsReq (Registry)
- App Restart Persistence   | Invoke-WA_SetRestartApps (Registry)
________________________________________________________

[M]aintenance
Orchestrated Maintenance:
- System Pre-Flight Check   | Invoke-WA_SystemPreCheck
- Windows Update (API/UIA)  | Invoke-WA_WindowsUpdate
- SFC System Scan           | Invoke-WA_WindowsRepair (Conditional: 30 days)
- DISM Repair               | Invoke-WA_WindowsRepair (Triggered on corruption)
...
System Hardening Check,SmartRUN,wa.ps1 (Embedded),Mixed,Checks system state vs desired configuration,N/A,No,Automation,Invoke-WinAutoConfiguration -SmartRun
Maintenance Cycle,SmartRUN,wa.ps1 (Embedded),Mixed,Checks Last Run dates (Repair=30d; Disk=7d; Clean=7d) to trigger tasks,N/A,No,Automation,Invoke-WinAutoMaintenance -SmartRun
...
Box Drive,Install,INSTALL_BoxDrive.ps1,ATOMIC_SCRIPT,MSI installer with specific logic,No,No,System,Invoke-WA_InstallApps
...
Real-Time Protection,Configure,wa.ps1 (Embedded),PS WMI,Set-MpPreference -DisableRealtimeMonitoring 0,Yes,No,Security,Invoke-WA_SetRealTimeProt
PUA Protection,Configure,wa.ps1 (Embedded),PS WMI,Set-MpPreference -PUAProtection 1,Yes,No,Security,Invoke-WA_SetPUABlockApps
PUA Protection (Edge),Configure,wa.ps1 (Embedded),Registry (HKCU),HKCU:\Software\Microsoft\Edge\SmartScreenPuaEnabled (1),Yes,No,Security,Invoke-WA_SetPUABlockDLs
Memory Integrity,Configure,wa.ps1 (Embedded),Registry (HKLM),HKLM:\SYSTEM\CurrentControlSet\Control\DeviceGuard\Scenarios\HypervisorEnforcedCodeIntegrity (1),Yes,Yes,Security,Invoke-WA_SetMemoryInteg
...
Defender Remediation (UIA),Configure,wa.ps1 (Embedded),UI Automation,Automates Windows Security Virus & Threat protection,No,No,Security,Invoke-WA_SetVirusThreatProtect
Microsoft Update Service,Configure,wa.ps1 (Embedded),Registry (HKLM),HKLM:\SOFTWARE\Microsoft\WindowsUpdate\UX\Settings\AllowMUUpdateService (1),Yes,No,Config,Invoke-WA_SetMicrosoftUpd
...
function Invoke-WA_InstallApps {
    ...
    $AppsToInstall = @()
    foreach ($app in $AppList) {
        if (-not (Test-WA_AppInstalled -App $app)) {
            $AppsToInstall += $app
        }
    }
    ...
    if (@($AppsToInstall).Count -gt 0) {
        ...
        foreach ($app in $AppList) {
            Write-Host ""
            if (Test-WA_AppInstalled -App $app) {
            ...
            try {
                ...
                elseif ($app.Type -eq "ATOMIC_SCRIPT") {
                    ...
                    $root = if ($PSScriptRoot) { $PSScriptRoot } else { $PWD.Path }
                    # Check direct installer paths and potential flat path
                    $pathsToCheck = @(
                        "scripts\Installers\$scriptName",
                        "Installers\$scriptName",
                        "scripts\$scriptName",
                        "$scriptName"
                    )
...
#>
    ...
    Write-LeftAligned "Starting Windows Security Automation (Kernel-mode Stack Protection)..."
    ...
    while (-not $Success -and ($RetryCount -lt $MaxRetries)) {
        ...
        Write-LeftAligned "Launching Windows Security (Iteration $RetryCount)..."
        ...
        if (-not $Window) {
            Write-LeftAligned "$FGDarkYellow$Char_Warn Could not find 'Windows Security' window. Retrying...$Reset"
            continue
        }
        Write-LeftAligned "$FGGreen$Char_HeavyCheck Found 'Windows Security' window.$Reset"
        try { $Window.SetFocus() } catch {}

        # 3. Navigate to "Device security"
        Write-LeftAligned "Navigating to 'Device security'..."
        ...
        if ($DeviceSecBtn) {
            ...
            Write-LeftAligned "$FGRed$Char_RedCross Could not find 'Device security' navigation item.$Reset"
            continue
        }

        # 4. Navigate to "Core isolation details"
        Write-LeftAligned "Navigating to 'Core isolation details'..."
        ...
        if ($CoreIsoLink) {
            ...
            Write-LeftAligned "$FGDarkYellow$Char_Warn Could not find 'Core isolation details' link. Checking if already there...$Reset"
        }

        # 5. Find Target Toggle
        Write-LeftAligned "Looking for 'Kernel-mode Hardware-enforced Stack Protection' toggle..."
        ...
        if ($TargetToggle) {
            Write-LeftAligned "Found Target Element. Checking state..."
            ...
            if ($State -eq $DesiredState) {
                Write-LeftAligned "$FGGreen$Char_HeavyCheck Feature is already $ActionStr.$Reset"
                $Success = $true
            }
            elseif ($null -ne $State) {
                # State matches 0 or 1 but is not desired
                Write-LeftAligned "Feature is $(if($State -eq 1){'ON'}else{'OFF'}). Toggling $ActionStr..."
                ...
                if (Invoke-UIAElement -Element $TargetToggle) {
                    ...
                    Write-LeftAligned "$FGDarkYellow$Char_Warn Toggle pattern failed. Attempting Invoke (Click)...$Reset"
                    try {
                        ...
                        Write-LeftAligned "$FGRed$Char_RedCross Invoke pattern also failed.$Reset"
                    }
                }

                if ($Toggled) {
                    Write-LeftAligned "$FGGreen$Char_HeavyCheck Action triggered. Waiting for update...$Reset"
                    Start-Sleep -Seconds 3
                 
                    $StateAfter = Get-UIAToggleState -Element $TargetToggle
                    if ($StateAfter -eq $DesiredState) {
                        Write-LeftAligned "$FGGreen$Char_HeavyCheck Successfully verified state is $ActionStr.$Reset"
                        $Success = $true
                    }
                    else {
                        Write-LeftAligned "$FGDarkYellow$Char_Warn State did not change. This is common if UAC is prompting or a reboot is pending.$Reset"
                        # We assume success if we clicked it, as we can't automate the UAC prompt easily.
                        $Success = $true 
                    }
                }
                else {
                    Write-LeftAligned "$FGRed$Char_RedCross Failed to interact with the toggle.$Reset"
                }
            }
            else {
                # No toggle pattern (e.g. Button?)
                Write-LeftAligned "$FGDarkYellow$Char_Warn Toggle state unknown. Attempting to Click...$Reset"
            ...
            Write-LeftAligned "$FGRed$Char_RedCross Could not find 'Kernel-mode Hardware-enforced Stack Protection' toggle. Feature might not be supported on this hardware.$Reset"
            $Success = $true 
        }
    }

    Write-LeftAligned "Automation complete."
}
