...
<#
.SYNOPSIS
    Enables Windows Firewall via UI Automation.
.DESCRIPTION
    Opens Windows Security > Firewall & network protection and clicks "Turn on" (or "Restore settings").
    Useful when one or more firewall profiles are off and a dashboard button is visible.
    Standalone version. Includes Reverse Mode (-r) stub.
.PARAMETER Reverse
    (Alias: -r) No-Op. Reversing requires SET_FirewallON.ps1 (logic to disable is different).
#>
...
& {
    ...
    # 1. Launch Windows Security at Firewall & network protection
